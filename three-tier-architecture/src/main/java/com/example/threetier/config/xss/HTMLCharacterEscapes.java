package com.example.threetier.config.xss;

import com.fasterxml.jackson.core.SerializableString;
import com.fasterxml.jackson.core.io.CharacterEscapes;
import com.fasterxml.jackson.core.io.SerializedString;
import lombok.extern.slf4j.Slf4j;
import org.owasp.encoder.Encode;

import java.util.regex.Pattern;

@Slf4j
public class HTMLCharacterEscapes extends CharacterEscapes {

    private final int[] asciiEscapes;

    // XSS처리 하지 않기 위한 PATTERN
    // \u00C0-\u017F : LATIN 보충 문자 (라틴 알파벳에 악센트, 변형이 붙은 문자)
    private static final Pattern EXCEPT_PATTERN = Pattern.compile("[\\u00C0-\\u017F]");

    public HTMLCharacterEscapes() {
        log.info("XSS 보호: HTML 문자 이스케이프 초기화");

        // ASCII 문자에 표준 JSON 이스케이프 사용
        asciiEscapes = CharacterEscapes.standardAsciiEscapesForJSON();

        // XSS 에서 가장 중요한 < , > 에 대한 설정
        asciiEscapes['<'] = CharacterEscapes.ESCAPE_CUSTOM;
        asciiEscapes['>'] = CharacterEscapes.ESCAPE_CUSTOM;

        asciiEscapes['('] = CharacterEscapes.ESCAPE_CUSTOM;
        asciiEscapes[')'] = CharacterEscapes.ESCAPE_CUSTOM;
        asciiEscapes['#'] = CharacterEscapes.ESCAPE_CUSTOM;

        log.info("XSS 보호: 특수 문자 이스케이프 설정 완료 (<, >, (, ), #)");
    }

    @Override
    public int[] getEscapeCodesForAscii() {
        return asciiEscapes;
    }

    @Override
    public SerializableString getEscapeSequence(int ch) {
        String str = Character.toString((char) ch);
        if (isInExceptionPattern((char)ch)) {
            log.debug("XSS 이스케이프: 예외 패턴 문자 '{}' - 이스케이프 없이 유지", str);
            return new SerializedString(str);
        }
        else {
            String encoded = Encode.forHtml(str);
            log.debug("XSS 이스케이프: 문자 '{}' -> '{}'", str, encoded);
            return new SerializedString(encoded);
        }
    }

    public static boolean isInExceptionPattern(char character) {
        String str = Character.toString(character);
        boolean matches = EXCEPT_PATTERN.matcher(str).matches();
        if (log.isDebugEnabled() && matches) {
            log.debug("XSS 이스케이프: 문자 '{}' (유니코드: \\u{})는 예외 패턴에 해당됨", 
                    str, Integer.toHexString(character | 0x10000).substring(1));
        }
        return matches;
    }
}
